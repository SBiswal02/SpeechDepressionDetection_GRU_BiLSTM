...
# import keras.backend as K #replace with tf.linalg.matmul
import sys
...
class ContextGating(layers.Layer):
    ...
    def call(self, inputs):
        ...
        gates = tf.linalg.matmul(inputs, self.gating_weights)
...
class NetVLAD(layers.Layer):
    ...
    def call(self, reshaped_input):
        ...
        activation = tf.linalg.matmul(reshaped_input, self.cluster_weights)
        ...
        vlad = tf.linalg.matmul(vlad, self.hidden1_weights)
...
class SoftDBoW(layers.Layer):
    ...
    def call(self, reshaped_input):
        ...
        bow = tf.linalg.matmul(bow, self.hidden1_weights)
        return bow
...
class NetFV(layers.Layer):
    ...
    def call(self, reshaped_input):
        ...
        fv = tf.linalg.matmul(fv, self.hidden1_weights)
        return fv
